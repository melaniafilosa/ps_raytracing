...
In the two-dimensional case, for every ray we need to choose one position coordinate $\variabile{x}_1$ and one angular coordinate $\optangle_1$, while the $\variabile{z}_1$-coordinate of every ray is always given (for instance, for the two-faceted cup in Figure \ref{fig:cup}, $\variabile{z}_1=0$). 
...
%Given \insieme{D}$=[-\variabile{a}, \variabile{a}]\times[-\pi/2, \pi/2]\subset\mathbb{R}^2$ where $\variabile{a}$ is the half length of the source, a 
 A set of random variables $\{\vect{Y}_1, \cdots, \vect{Y}_N\}$ is chosen such that they are uniformly distributes at the source, where $\vect{Y}_{\variabile{i}}= (\variabile{x}_{1}^{\variabile{i}},\optangle_1^{\variabile{i}})$.
...
Considering a sample of $\nrays$ rays, thnrayse variable $\variabile{Y}_{\variabile{j}} = \sum_{\variabile{k}=1}^{\nrays} \chi_{\variabile{j}}(\optangle^{\variabile{k}})$
...
\begin{figure}[h]
\begin{center}
    \includegraphics[width=0.7\textwidth]{Error_cup}
