...
% \cite{joe2008notes, leobacher2014introduction} \cite{leobacher2014introduction}\cite{joe2008constructing}
...
In Quasi-Monte Carlo (QMC) ray tracing the rays are chosen from a so-called low discrepancy sequence \cite{joe2008notes,joe2008constructing}. This method is discussed in Section \ref{sec:QMC}.
\section{Monte Carlo (MC) ray tracing}
MC \textit{integrations} rely on stochastic random samples to obtain numerical results for a deterministic problem. In particular, MC simulations can be a very easy way to solve physics problems based on numerical integration \cite{jensen2003monte}. \textit{Ray tracing} provides a light distribution at the target given an initial light distribution at the source and an optical system.
...
Applying ray tracing to the initial random set of rays, a random rays distribution at the target of the system is obtained. From this distribution, we can calculate, for instance, the profile of the target intensity using MC approximation. The differences and similarities between MC integration and MC ray tracing is better explained in Appendix \ref{chap:appraytracing}.
...
There are many ways to generate low-discrepancy sequences \cite{dalal2008low}. The most common QMC approach uses the so-called Sobol sequence \cite{joe2008constructing}. The algorithm for generating Sobol sequences is widely explained in the literature, (see for instance \cite{bratley1988algorithm}). When using Sobol sequences, the absolute value of QMC error can be estimated by:
\begin{equation}
