...
$[\vect{a}, \vect{b}]  = [a_1, b_1] \times [a_2, b_2]$. Given a function $f:[\vect{a},\vect{b}]\subset \mathbb{R}^2 \mapsto \mathbb{R}$ and a random variable 
...
\end{equation}
where $\{\vect{y}_\variabile{i}\}_{\variabile{i} = 1, \cdots, \const{N}}\in D$ are independent samples of the probability density function  $\rho$, \cite{owen2003quasi}. Thus MC is based on the following approximation:
...
\mathbb{E}[S_{N}(f)] = \frac{1}{N}\sum_{\variabile{i}=1}^{\textrm{N}}\mathbb{E}[f] = \mathbb{E}[f],
\end{equation}
...
\textrm{Var}(S_{N})= \textrm{Var}\Bigg(\frac{1}{N}\sum_{\variabile{i}=1}^{N}f(\vect{y}_\variabile{i})\Bigg) =
 \frac{1}{N^2}\sum_{\variabile{i}=1}^{N}\textrm{Var}\big(f(\vect{y}_\variabile{i})\big)
\end{equation}
which can be applied because the variables $\{\vect{y}_\variabile{i}\}_{i = 1, \cdots, N}$ are independent \cite{grinstead2012introduction}. 
...
Indicating with $\variabile{x}_{1}^{\variabile{k}}$ $\variabile{x}$ position coordinate of the $\variabile{k}$-th ray and with $\optangle_1^{\variabile{k}}$ its direction coordinate, a set of random variables $\{\vect{y}_1, \cdots, \vect{y}_N\}\in [\vect{a}, \vect{b}]\subset\mathbb{R}^2$ is chosen such that $\vect{y}_{\variabile{k}}= (\variabile{x}_{1}^{\variabile{k}},\optangle_1^{\variabile{k}})$.
...
In particular, we are interested in the total target intensity $I$ as a function of the angular coordinate $\optangle$ which is given by:
\begin{equation}
I(\optangle) = \int_{\variabile{x}^{\textrm{min}}}^{\variabile{x}^{\textrm{max}}}L(\variabile{x}, \optangle)\textrm{d}\variabile{x}\,
\end{equation}
where $\variabile{x}^{\textrm{min}}$ and $\variabile{x}^{\textrm{max}}$ are the minimum and the maximum position coordinates of all the possible position of the rays at the target. Using the MC average, the previous integral is approximated by:
\begin{equation}
\int_{\variabile{x}^{\textrm{min}}}^{\variabile{x}^{\textrm{max}}}L(\variabile{x}, \optangle)\textrm{d}\variabile{x} = \frac{1}{N}\sum_{\variabile{i}=1}^{N}L(\variabile{x}^{\variabile{i}}, \optangle^{\variabile{i}}).
\end{equation}
\\ \indent The target is divided into intervals of equal length, the so-called bins. A partitioning
...
$I(\optangle)$, which is continuous in $[\optangle_{\variabile{j}-1}, \optangle_{\variabile{j}}]$, there exists a value $\optangle_{\variabile{j}}\in[\optangle_{\variabile{j}-1}, \optangle_{\variabile{j}}]$ for which the integral at the numerator in Equation (\ref{eq:probability}) can be written as:
...
We conclude this section implementing MC ray tracing for the two-faceted cup, the profile of which is depicted in Fig. \ref{fig:cup}. 
...
at the source, we obtain an example of the ray distribution on the $(\variabile{x}, \optangle)$-plane shown in Fig. \ref{fig:mc_sample}. 
...
The profile of $\hat{I}_{\const{MC}}$ is depicted in Fig. \ref{fig:mc_intensity} with the red line. The exact intensity is shown with the green line in the same figure.
\begin{figure}[t]
\begin{center}
...
Instead of considering random variable the sample of rays can be defined such that they are regularly distributed on the domain $D\subseteq\mathbb{R}^2$ of $f$. Methods based on this deterministic approach are called Quasi-Monte Carlo (QMC) methods. They can be seen as an improvement of MC method. The ray tracing procedure that considers such rays distribution is called QMC ray tracing.
...
\end{equation} that is the number of $\vect{y}_{\variabile{n}}$ such that $\vect{y}_{\variabile{n}}\in [\vect{a},\vect{b})$ and $\lambda_([\vect{a},\vect{b})) =(b_1-a_1)\times (b_2-a_2)$.
\end{definition}
...
Often, it is enough to consider the discrepancy in the subset $[\vect{a},\vect{b})\subseteq[0,1)^2$ with $\vect{a}=0$, in which case we talk about star discrepancy.
 \begin{definition}
Let $\mathcal{S} = \{\vect{y}_1, \cdots, \vect{y}_N\}$ be a set of $N$ points in $[0,1)^d$. The  star discrepancy $D^*_N({\mathcal{S}})$ of $\mathcal{S}$ is defined as:
\begin{equation}
...
\begin{equation}
\Bigg|\int_{[0,1)^2}f(\vect{y}) \textrm{d}\vect{y}-\frac{1}{N}\sum_{\variabile{i}=1}^{N}f(\vect{y}_\variabile{i})\Bigg|\leq V(f)D^*_N({\mathcal{S}}),
\end{equation}
% Explain better the variation function
where $V(f)$ is the so-called variation function of $f$ in the sense of Hardy-Krause (see \cite{brandolini2013koksma,owen2005multidimensional} for details). It is important to remark that for many function $f$ arising can be that $V(f)=\infty$ \cite{wang2008low}. For the functions we analyze in this thesis the corresponding variation function is always bounded. In those cases, from the definition of low-discrepancy sequences and from the Koksma-Hlawka inequality follows:
\begin{equation}
...
The convergence of QMC methods depend on the low-discrepancy sequence that is used.\\ \indent
There are many ways to generate low-discrepancy sequences \cite{dalal2008low}.
...
Given, for instance, a Sobol sequence $\{\vect{y}_{\variabile{i}}\}_{\variabile{i}=1, \cdots, N}$ with $\vect{y}_\variabile{i}\in[0,1)^2$ for every $\variabile{i}=1, \cdots, N$, the two dimensional QMC ray tracing the coordinates $(\variabile{x}_1^{\variabile{k}}, \optangle_1^{\variabile{k}})$ of the $\variabile{k}$-th ray at the source equal to those of the $\variabile{k}$-th points of the sequence, i.e., $(\variabile{x}_1^{\variabile{k}}, \optangle_1^{\variabile{k}})=\vect{y}_{\variabile{k}}$. 
$\vect{y}_{\variabile{i}}$ of the Sobol sequence  $\{\vect{y}_{\variabile{i}}\}_{\variabile{i}=1, \cdots, N}$ and, the direction coordinate of the $\variabile{i}$-th ray at the source equal to the second component of the $\variabile{i}$-th point 
$\vect{y}_{\variabile{i}}$ of the same sequence. A set of $\nrays = N$ rays with these initial coordinates is traced within the system and, once the target coordinates of all the rays traced are computed, the output intensity is calculated like for MC ray tracing by dividing the target into bins and counting, on average, the number rays that follow into each bin. Thus, the only difference between MC and QMC ray tracing consists only on the choice of the initial ray set. 
Like for MC ray tracing we can derive the dependence of the error as a function of the number of bins $\nbin$. Since the discretization error (first term on the right hand side of inequality (\ref{eq:error_int})) does not depend on MC noise, we expect that it does not change in QMC error. Hence we can predict an error convergence for QMC ray tracing given by:
\begin{equation} \begin{aligned}
\Big|I(\optangle_{\variabile{j}-1/2})&-\frac{\Phi}
{\Delta\optangle}I_{\const{QMC}}(\optangle_{\variabile{j}-1/2})\Big| \leq
\frac{\const{C}_1}{\nbin^2} + \const{C}_2\frac{\log(\nrays)^2}{\nrays},
\end{aligned}
\end{equation}
...
We will show in this thesis that phase space ray tracing allows to trace a relatively small number of rays inside the system to obtain the desired accuracy of the target intensity. 
