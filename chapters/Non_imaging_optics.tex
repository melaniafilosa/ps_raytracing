...
\end{equation}
for computing the direction of the refracted ray, we need to specify the direction of $\boldsymbol{\nu}$ which is usually chosen so that the scal product between the incident ray and the normal is negative $\vect{t}_{\textrm{i}}\boldsymbol{\cdot}\boldsymbol{\nu}<0$. 
...
\section{Fresnel's equations for reflection and refraction}\label{sec:fresnel}
...
$|\vect{k}| = k$, \vect{x} is the position vector and $\mytime$ is the time. The amplitude $\boldsymbol{\mathcal{E}}_0(\vect{x})$ is constant in time and $\omega = \frac{c |\vect{k}|}{n}$ is the value of the angular frequency with $c$ the velocity of light in vacuum and $\n$ the index of refraction in which the wave is traveling, which is the ratio of $c$ and the speed of light $v$ in the material. Note that the angular frequency can be also written as $\omega = v\,k$ (in vacuum $\n=1$ and $\omega=c\,k$). The value
...
\indent Fresnel's equations were introduced to describe the effect of an incident wave when encountering an interface located between two media having different indices of refraction. In particular, Fresnel coefficients determine the fractions of transmitted and reflected energy.  
...
\label{fig:electric_field_p}
 \end{figure}
The amplitude Fresnel coefficient $r$ are defined by the ratio between the amplitudes of the reflected and the incident electric field. Similarly, $t$ is the ratio between the amplitudes of the transmitted and the incident electric field. They are defined by:
\begin{equation}
\begin{split}
...
\end{equation}
Most often used optical materials are non-magnetic, hence we assume $\mu_{\textrm{i}}=\mu_{\textrm{t}}=\mu_{0}$ \cite{lvovsky2013fresnel}. Employing (\ref{Econservation}) we arriving at the Fresnel coefficient for s-polarized light:
...
The boundary conditions for the electric and magnetic field:
\begin{subequations}
\begin{align}
...
\end{align}
\end{subequations}
leads to the second pair of Fresnel's equations:
...
\end{minipage}}
\end{equation}
where $\mathcal{R}_\textrm{p}$, $\mathcal{R}_\textrm{s}$, $\mathcal{T}_\textrm{p}$ and $\mathcal{T}_\textrm{s}$ are obtained from (\ref{Fresnel_pands})
